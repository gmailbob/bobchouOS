...
static void uart_putnum(int64 n) {
    if (n < 0) {
        uart_putc('-');
        n = -n;
    }
...
static void uart_puthex(uint64 n) {
    uart_puts("0x");
...
static void check_uint64(const char *name, uint64 got, uint64 expected) {
    tests_total++;
    if (got == expected) {
        tests_passed++;
        uart_puts("  PASS  ");
...
static void check_mem(const char *name, const void *got, const void *expected, uint64 n) {
    tests_total++;
...
static void test_bitfield_extract(void) {
    ...
    check_uint64("extract(0xABCD, 8, 4)",
                 bitfield_extract(0xABCD, 8, 4), 0xB);
    check_uint64("extract(0xABCD, 0, 8)",
                 bitfield_extract(0xABCD, 0, 8), 0xCD);
    check_uint64("extract(0xABCD, 4, 8)",
                 bitfield_extract(0xABCD, 4, 8), 0xBC);
...
static void test_bitfield_insert(void) {
    ...
    check_uint64("insert(0, 60, 4, 0xD)",
                 bitfield_insert(0, 60, 4, 0xD), 0xD000000000000000UL);
}
...
static void list_to_str(struct node *head, char *buf, int bufsize) {
    char *p = buf;
    ...
    while (head && p < end - 4) {
        ...
        if (v >= 100 && p < end) *p++ = '0' + (v / 100) % 10;
        if (v >= 10 && p < end)  *p++ = '0' + (v / 10) % 10;
        if (p < end) *p++ = '0' + v % 10;
...
static void test_format_hex(void) {
    ...
    format_hex(buf, 0);
    check_str("format_hex(0)",
              buf, "0x0000000000000000");

    format_hex(buf, 255);
    check_str("format_hex(255)",
              buf, "0x00000000000000FF");

    format_hex(buf, 0xDEADBEEF);
    check_str("format_hex(0xDEADBEEF)",
              buf, "0x00000000DEADBEEF");

    format_hex(buf, 0x10000000);
    check_str("format_hex(0x10000000)",
              buf, "0x0000000010000000");

    format_hex(buf, 0xFFFFFFFFFFFFFFFFUL);
    check_str("format_hex(0xFFFF...)",
